...
// this is OpenSSL-specific and the test is skipped for GnuTLS.
//
...
SEASTAR_THREAD_TEST_CASE(test_concurrent_put_with_key_update) {
    if (using_gnutls()) {
        // GnuTLS does not emit rehandshake/key-update output from the read
        // path, so the concurrent-put scenario does not apply.
        return;
    }
    ...
    b.set_x509_trust_file(certfile("catest.pem"), tls::x509_crt_format::PEM).get();
    b.set_dh_level();
    // force TLS 1.3 so that force_rehandshake() turns into a key-update, which
    // is what makes the OpenSSL read path produce output.
