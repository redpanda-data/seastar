...
// ===== Redpanda-derived coroutine benchmarks =====
...
// per field.  This is the pattern used in Redpanda's record multiplexer
// for protobuf/avro/json field processing.  The outer loop amortizes
// measurement overhead across CORO_BENCH_ITERS records.
...
PERF_TEST_F(coro_bench, empty_cont) { return co_await_in_loop(bench_empty_cont); }

PERF_TEST_F(coro_bench, ss_now) { return co_await_in_loop(bench_ss_now); }

PERF_TEST_F(coro_bench, ss_now_collect) { return collect_futures(bench_ss_now); }
...
PERF_TEST_F(coro_bench, per_field_1) {
    return co_await_per_field_direct<1>();
}

PERF_TEST_F(coro_bench, per_field_10) {
    return co_await_per_field_direct<10>();
}

PERF_TEST_F(coro_bench, per_field_40) {
    return co_await_per_field_direct<40>();
}

// ===== End redpanda-derived benchmarks =====
