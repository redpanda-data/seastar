...
template <> struct fmt::formatter<seastar::tls::openssl_session> : public fmt::formatter<string_view> {
    auto format(const seastar::tls::openssl_session& s, fmt::format_context& ctx) const -> decltype(ctx.out());
};
...
template<>
struct is_error_code_enum<seastar::openssl_errc> : true_type {};

}

template<>
struct fmt::formatter<seastar::openssl_errc> :
  ...
  alternate_message(seastar::openssl_errc error);

  auto format(seastar::openssl_errc error, fmt::format_context& ctx) const -> decltype(ctx.out());
};

namespace seastar {

int tls_version_to_openssl(tls::tls_version version) {
    switch(version) {
...
public:
    constexpr openssl_error_category() noexcept : std::error_category{} {}
    const char* name() const noexcept override {
        return "OpenSSL";
    }
    std::string message(int error) const override {
        return fmt::format("{}", static_cast<openssl_errc>(error));
    }
};

static const std::error_category& openssl_error_cat() {
    static const openssl_error_category ec;
    return ec;
}
...
std::vector<openssl_errc> get_all_openssl_errors() {
    ...
    for (auto code = ERR_get_error(); code != 0; code = ERR_get_error()) {
        error_codes.push_back(static_cast<openssl_errc>(code));
    }

    return error_codes;
}

std::system_error make_openssl_error(const std::string & msg, std::vector<openssl_errc> error_codes) {
...
std::system_error make_openssl_error(const std::string & msg) {
...
std::vector<unsigned char> make_openssl_alpn_proto_data(const std::vector<sstring>& protocols) {
...
private:
     ...
     * @throws openssl_error if unable to generate random data
     */
    template<std::size_t N>
    static void generate_key(std::array<uint8_t, N>& key) {
        if (RAND_priv_bytes(key.data(), N) <= 0) {
            throw make_openssl_error("Failed to generate key");
        }
    }
};
...
///
class openssl_provider_dh_params_impl : public tls::dh_params_impl {
public:
    using level = tls::dh_params::level;

    explicit openssl_provider_dh_params_impl(level) {}
    openssl_provider_dh_params_impl(const tls::blob&, tls::x509_crt_format){}
...
public:
    ...
    static int verify_callback(int preverify_ok, X509_STORE_CTX* store_ctx) {
        ...
        auto credential_impl = static_cast<openssl_provider_certificate_credentials_impl*>(X509_STORE_get_ex_data(store, credential_store_idx));
    ...
    openssl_provider_certificate_credentials_impl() : _creds([] {
    ...
    static void iterate_pem_certs(const bio_ptr& cert_bio, LoadFunc fn) {
        ...
        if (num_elements <= 0) {
            throw make_openssl_error("Failed to parse PEM cert");
        }
    ...
    static x509_ptr parse_x509_cert(const tls::blob& b, tls::x509_crt_format fmt) {
        ...
        if (!cert) {
            throw make_openssl_error("Failed to parse x509 certificate");
        }
        return cert;
    }

    void set_x509_trust(const tls::blob& b, tls::x509_crt_format fmt) override {
        bio_ptr cert_bio(BIO_new_mem_buf(b.data(), b.size()));
        x509_ptr cert;
        switch(fmt) {
        case tls::x509_crt_format::PEM:
            iterate_pem_certs(cert_bio, [this](X509_INFO* info){
                if (!info->x509) {
                    throw make_openssl_error("Failed to parse x509 cert");
                }
    ...
    void set_x509_crl(const tls::blob& b, tls::x509_crt_format fmt) override {
        bio_ptr cert_bio(BIO_new_mem_buf(b.data(), b.size()));
        x509_crl_ptr crl;
        switch(fmt) {
        case tls::x509_crt_format::PEM:
            iterate_pem_certs(cert_bio, [this](X509_INFO* info) {
                if (!info->crl) {
                    throw make_openssl_error("Failed to parse CRL");
                }
        ...
        case tls::x509_crt_format::DER:
            ...
            if (!crl) {
                throw make_openssl_error("Failed to parse x509 crl");
            }
    ...
    void set_x509_key(const tls::blob& cert, const tls::blob& key, tls::x509_crt_format fmt) override {
        ...
        case tls::x509_crt_format::PEM:
            ...
            iterate_pem_certs(bio_ptr{BIO_new_mem_buf(cert.data(), cert.size())}, [this, &x509_cert](X509_INFO* info) {
                if (!info->x509) {
                    throw make_openssl_error("Failed to parse X.509 certificate in loading key/cert chain");
                }
                if (!x509_cert) {
        ...
        if (!pkey) {
            throw make_openssl_error("Error attempting to parse private key");
        }
        if (!X509_check_private_key(x509_cert.get(), pkey.get())) {
            throw make_openssl_error("Failed to verify cert/key pair");
        }
        _cert_and_key = certkey_pair{.cert = std::move(x509_cert), .key = std::move(pkey)};
    }

    void set_simple_pkcs12(const tls::blob& b, tls::x509_crt_format, const sstring& password) override {
        ...
        if (auto p12 = pkcs12(d2i_PKCS12_bio(bio.get(), nullptr))) {
            ...
            if (!PKCS12_parse(p12.get(), password.c_str(), &pkey, &cert, &ca)) {
                throw make_openssl_error("Failed to extract cert key pair from pkcs12 file");
            }
            ...
            throw make_openssl_error("Failed to parse pkcs12 file");
        }
    }
    ...
    tls::session_resume_mode get_session_resume_mode() {
    ...
    void set_minimum_tls_version(tls::tls_version version) override {
        _min_tls_version.emplace(version);
    }

    void set_maximum_tls_version(tls::tls_version version) override {
    ...
    const std::optional<tls::tls_version>& minimum_tls_version() const noexcept {
        return _min_tls_version;
    }

    const std::optional<tls::tls_version>& maximum_tls_version() const noexcept {
...
private:
    ...
    tls::dn_callback _dn_callback;
    std::optional<tls::tls_version> _min_tls_version;
    std::optional<tls::tls_version> _max_tls_version;
...
public:
    ...
    openssl_session(session_type t, shared_ptr<tls::certificate_credentials> creds,
      ...
      , _type(t) {
        ...
        if (1 != BIO_ctrl(out_bio.get(), BIO_C_SET_POINTER, 0, this)) {
            throw make_openssl_error("Failed to set bio ptr to out bio");
        }
        ...
        if (_type == session_type::CLIENT && !_options.session_resume_data.empty()) {
            ...
            if (!sess) {
                throw make_openssl_error("Failed to decode SSL_SESSION data for session resumption");
            }
            if (1 != SSL_set_session(_ssl.get(), sess.get())) {
                throw make_openssl_error("Failed to set SSL_SESSION on SSL for session resumption");
            }
        }
        _options.session_resume_data.clear();
    }

    openssl_session(session_type t, shared_ptr<certificate_credentials> creds,
            connected_socket sock,
            tls_options options = {})
            : openssl_session(t, std::move(creds), net::get_impl::get(std::move(sock)), options) {}

    ~openssl_session() {
        SEASTAR_ASSERT(!_output_in_flight);
    }
    ...
    // Waits until the put() currently in flight on the underlying socket has
    // drained; if it failed, the error is saved into _error and returned.
    //
    // The wait is driven by a condition variable rather than by consuming a
    // future, because it is called from both the read and write paths (which
    // use different semaphores, _in_sem vs _out_sem, and can both emit output
    // -- e.g. a write encrypting application data, and a read that has to emit
    // a TLS key-update/renegotiation message). _output_in_flight stays true
    // until the put actually drains, so the guard in bio_write_ex() can't be
    // fooled into issuing a second, concurrent put() on the same data_sink
    // (which would trip SEASTAR_ASSERT(!_p) in posix_data_sink_impl).
    future<> wait_for_output() {
        tls_log.trace("{} wait_for_output", *this);
        return _output_idle.wait([this] { return !_output_in_flight; }).then([this] {
            if (_output_error) {
                auto ep = std::exchange(_output_error, std::exception_ptr());
    ...
    future<> do_put_one(const char* ptr, size_t size) {
        ...
        // Drain any output that may still be in flight before we drive
        // SSL_write_ex (which produces more output). Output can be issued by
        // the read path too -- e.g. a TLS key-update/renegotiation message
        // emitted while processing an incoming record -- and that path holds
        // _in_sem, not _out_sem, so holding _out_sem here is not enough to
        // guarantee _out is idle. Waiting keeps puts on the underlying sink
        // strictly sequential across both paths.
                        ...
                        case SSL_ERROR_SSL:
                        {
                            ...
                            if (contains_openssl_error(error_codes, ERR_LIB_SSL, SSL_R_PEER_DID_NOT_RETURN_A_CERTIFICATE) ||
                                ...
                                try {
                                    ...
                                    // Flush pending output (e.g. TLS alert) so the peer
                                    // learns about the failure, then propagate the error.
    ...
    future<> force_rehandshake() override {
        ...
        if (_type == session_type::CLIENT) {
            throw std::runtime_error("re-handshake only applicable for server socket");
        }
        // TLS 1.3 does not support renegotiation. Use key update instead,
        // which rotates the session keys. This is the OpenSSL equivalent
        // of GnuTLS's gnutls_rehandshake for TLS 1.3.
        return with_semaphore(_out_sem, 1, [this] {
            if (!SSL_key_update(_ssl.get(), SSL_KEY_UPDATE_REQUESTED)) {
                throw make_openssl_error("SSL_key_update failed");
            }
            return wait_for_output();
        });
    }
...
private:
    ...
    static std::optional<sstring> get_dn_string(X509_NAME* name, dn_format format = dn_format::legacy) {
        ...
        if (len < 0) {
            throw make_openssl_error("Failed to allocate DN string");
        }
...
private:
    ...
    // Output state for the underlying socket (_out). Only one put() may be in
    // flight on a data_sink at a time, but output is issued from both the read
    // and write paths (see wait_for_output()), so a single flag -- not
    // semaphore ownership -- tracks whether _out is busy:
    //   _output_in_flight  the guard bio_write_ex() consults; stays true until
    //                      the in-flight put drains.
    //   _output_error      failure of the in-flight put, surfaced by the next
    //                      wait_for_output().
    //   _output_idle       broadcast when a put completes, so any number of
    //                      waiters (read path and write path) can await it.
    bool _output_in_flight = false;
    std::exception_ptr _output_error;
    condition_variable _output_idle;
    buf_type _input;
...
tls::openssl_session* unwrap_bio_ptr(void * ptr) {
    return static_cast<tls::openssl_session*>(ptr);
}

tls::openssl_session* unwrap_bio_ptr(BIO * b) {
...
long bio_ctrl(BIO * b, int ctrl, long num, void * data) {
    ...
    case BIO_CTRL_WPENDING:
        return session->_output_in_flight ? 1 : 0;
    default:
        return 0;
    }
...
///
/// This function attempts to call _out.put(). Only one put may be in flight at
/// a time: if a previous put is still draining (_output_in_flight), it returns
/// '0' and sets the retry-write flag so OpenSSL retries once the put completes
/// (driven by the caller's wait_for_output()).
int bio_write_ex(BIO* b, const char * data, size_t dlen, size_t * written) {
    auto session = unwrap_bio_ptr(b);
    tls_log.trace("{} bio_write_ex: dlen {}", *session, dlen);
    BIO_clear_retry_flags(b);

    if (session->_output_in_flight) {
        tls_log.trace("{} bio_write_ex: output still draining", *session);
    ...
    try {
        ...
        if (!session->_output_error) {
            auto buf = temporary_buffer<char>(dlen);
            std::memcpy(buf.get_write(), data, dlen);
            n = buf.size();
            auto f = session->_out.put(std::move(buf));
            if (f.available()) {
                // Completed synchronously: surface a failure right away (caught
                // below); on success _out is already idle, nothing is pending.
                if (f.failed()) {
                    std::rethrow_exception(std::move(f).get_exception());
                }
            } else {
                // Drains in the background; mark _out busy and wake any waiters
                // when it completes. Hold a reference to the session since the
                // completion handler touches its state.
                session->_output_in_flight = true;
                (void)std::move(f).then_wrapped(
                    [self = session->shared_from_this()](future<> f) {
                        self->_output_in_flight = false;
                        if (f.failed()) {
                            self->_output_error = std::move(f).get_exception();
                        }
                        self->_output_idle.broadcast();
                    });
            }
            tls_log.trace("{} bio_write_ex: queued {} bytes to output", *session, n);
        }

        if (session->_output_error) {
            tls_log.debug("{} bio_write_ex: output pending has error", *session);
            std::rethrow_exception(session->_output_error);
        }
        ...
        session->_output_error = std::current_exception();
    } catch(...) {
...
bio_method_ptr create_bio_method() {
    ...
    if (new_index == -1) {
        throw make_openssl_error("Failed to obtain new BIO index");
    }
    bio_method_ptr meth(BIO_meth_new(new_index, "seastar-openssl"));
    if (!meth) {
        throw make_openssl_error("Failed to create new BIO method");
    }

    if (1 != BIO_meth_set_create(meth.get(), bio_create)) {
        throw make_openssl_error("Failed to set the BIO creation method");
    }

    if (1 != BIO_meth_set_ctrl(meth.get(), bio_ctrl)) {
        throw make_openssl_error("Failed to set BIO control method");
    }

    if (1 != BIO_meth_set_write_ex(meth.get(), bio_write_ex)) {
        throw make_openssl_error("Failed to set BIO write_ex method");
    }

    if (1 != BIO_meth_set_read_ex(meth.get(), bio_read_ex)) {
        throw make_openssl_error("Failed to set BIO read_ex method");
    }
...
std::unique_ptr<tls::dh_params_impl> tls::openssl::make_dh_params(tls::dh_params::level lvl) {
    return std::make_unique<openssl_provider_dh_params_impl>(lvl);
}
...
auto fmt::formatter<seastar::tls::openssl_session>::format(
    const seastar::tls::openssl_session& s, fmt::format_context& ctx) const -> decltype(ctx.out()) {

    return fmt::format_to(ctx.out(), "{}:{}:{} -",
        s.get_type_string(), s.local_address(), s.remote_address());
}

std::optional<seastar::sstring> fmt::formatter<seastar::openssl_errc>::alternate_message(seastar::openssl_errc error) {
...
auto fmt::formatter<seastar::openssl_errc>::format(seastar::openssl_errc error, fmt::format_context& ctx) const -> decltype(ctx.out()) {
